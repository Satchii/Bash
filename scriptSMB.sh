...
# Récupérer la liste de tous les utilisateurs Nextcloud actifs
...
for USER_ID in $USERS; do 
    ...
    MOUNT_EXISTS=$(sudo -u "$NEXTCLOUD_WEB_USER" php "$NEXTCLOUD_PATH"/occ files_external:list "$USER_ID" --output=json |\
        jq -r --arg user_id "$USER_ID" '[.[] | select(.backend == "smb" and .configuration.share == $user_id and (.applicable_users | contains([$user_id]))) ] | length')
